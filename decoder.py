...
from inception import inception, upsample_map, conv_w_init, conv_b_init


def decoder(name, z, phase, shortcut=False, reg=None):
    ...
    with tf.name_scope(name):
        ...
        with tf.name_scope('feature8'):
            feature = upsample_map(feature)
            feature = inception('feature8_a', feature, phase, 256, 128, 160, 320, 32, 128, shortcut, reg)
            feature = inception('feature8_b', feature, phase, 256, 128, 160, 320, 32, 128, shortcut, reg)

        # feature16
        with tf.name_scope('feature16'):
            feature = upsample_map(feature)
            feature = inception('feature16_a', feature, phase, 128, 64, 128, 224, 32, 96, shortcut, reg)
            feature = inception('feature16_b', feature, phase, 128, 64, 128, 224, 32, 96, shortcut, reg)

        # feature32
        with tf.name_scope('feature32'):
            feature = upsample_map(feature)
            feature = inception('feature32_a', feature, phase, 64, 32, 64, 112, 16, 48, shortcut, reg)
            feature = inception('feature32_b', feature, phase, 64, 32, 64, 112, 16, 48, shortcut, reg)

        # feature64
        with tf.name_scope('feature64'):
            feature = upsample_map(feature)
            feature = inception('feature64_a', feature, phase, 32, 16, 32, 56, 8, 24, shortcut, reg)
            feature = inception('feature64_b', feature, phase, 32, 16, 32, 56, 8, 24, shortcut, reg)

        # feature128
        with tf.name_scope('feature128'):
            feature = upsample_map(feature)
            feature = inception('feature128_a', feature, phase, 16, 8, 16, 28, 4, 12, shortcut, reg)
            feature = inception('feature128_b', feature, phase, 16, 8, 16, 28, 4, 12, shortcut, reg)
