...
#    model = VaeSimple(input_dim, latent_dim, encoder_dim, decoder_dim, sample_num)
    model = VaeScale(input_dim, latent_dim, encoder_dim, decoder_dim, sample_num, gamma=gamma)
    ...
    with tf.Session() as sess:
        ...
        for stage in range(stage_num):
            ...
            num_epoch = 3**stage
            print('Stage = {0}, lr = {1}, num_epoch = {2}.'.format(stage, lr, num_epoch))
            train_model(model, x_train, lr, num_epoch, batch_size, sess, writer)
